...
const SITE_URL = 'https://klinikformanuelterapi.dk';

function buildLdJson(ratingValue: string, ratingCount: number): string {
...
export default function HeadMeta({ t, title, description, ogImage }: HeadMetaProps) {
  useEffect(() => {
    ...
    const ldJson = buildLdJson(t.rating.value, t.rating.count);
    ...
    if (ld.textContent !== ldJson) ld.textContent = ldJson;
